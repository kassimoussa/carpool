<div>
    <form wire:submit.register.prevent="save" {{-- action="{{ url('/fiche/store') }}" --}} role="form" method="post">
        ...
        <div class="@if ($currentStep != 1) hidden @endif container mx-auto w-full md:w-2/3 mb-4">
            ...
            <div class="flex mt-4">
                <div class="relative w-1/2" x-data="{ vDepart: true, vdp: '' }" @click.away="vDepart = false">
                    <div class=" relative ">
                        ...
                        <input type="text" id="input-group-1" wire:model="ville_depart" x-model="vdp" x-ref="vdp"
                            ...
                            class=" @error('ville_depart') {{ $errorColor }} @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm md:text-lg rounded-l-lg   focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-4  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                ...
                                <ul>
                                    @foreach ($ville_departs as $ville)
                                        <li wire:click="select_vdp('{{ $ville->nom_ville }}')"
                                            ...
                                            class="rounded text-white text-sm md:text-lg block p-2  hover:bg-gray-700">
                                            {{ $ville->nom_ville }}</li>
                                    @endforeach
                                </ul>
                            @endif
        ...
        <div class="@if ($currentStep != 2) hidden @endif container mx-auto w-full md:w-2/3 mb-4">
            ...
            <div class="flex mt-4">
                <div class="relative w-1/2" x-data="{ vDestination: true, vdt: '' }" @click.away="vDestination = false">
                    <div class=" relative ">
                        <div class="flex absolute inset-y-0 left-0 items-center pl-3 pointer-events-none">
                            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                class="w-5 h-5 text-gray-500 dark:text-gray-400" fill="currentColor"
                        ...
                        <input type="text" id="input-group-1" wire:model="ville_destination" x-model="vdt"
                            ...
                            class=" @error('ville_destination') {{ $errorColor }} @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm md:text-lg rounded-l-lg   focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-4  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        {{-- @if ($currentStep == 4) --}}
        <div class="@if ($currentStep != 4) hidden @endif container mx-auto relative w-full md:w-2/3 mb-4"
            x-data="{ open: false }">
            <div class=" text-center title mt-2 ">
            ...
            <div class="flex mx-auto  mt-4 ">
                <div class="relative w-1/2">
                    ...
                    <div x-show="open" @click.outside="open = false" id="passager_counter"
                        class=" z-50 absolute mt-4 w-44 bg-gray-50 rounded divide-y  ">
        ...
        @if ($voitures)
        <div class="@if ($currentStep != 5) hidden @endif container mx-auto relative w-full md:w-2/3 mb-4" >
            ...
            @foreach ( $voitures as $voiture)
                <label for="{{ $voiture->id }}" class="flex justify-between hover:bg-gray-600 p-2 items-center  mb-2">
                    <div class="flex flex-col">
                        <h3 class="text-lg text-white font-semibold "> {{ $voiture->marque }} {{ $voiture->model }}</h3>
                        <h3 class="text-md text-gray-200 font-normal"> {{ $voiture->couleur }}</h3>
                    </div> 
